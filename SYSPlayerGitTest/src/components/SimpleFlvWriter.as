package components
{
	...
	{			
		...
		public function SimpleFlvWriter(frameRate:int)
		{
			this.frameRate = frameRate;
		}

		public function createFile(pFile:File, pWidth:int, pHeight:int, pDurationInSeconds:Number=0):void
		{
			/*
				Parameters:
		...
		public function saveFrame(pBitmapData:BitmapData, currentTime:Number):void
		{
		...
		private function flvTagVideo():ByteArray
		{
			...
			var timeStamp:Number = currentTime; 
			trace("timeline:" + timeStamp.toString());
			// tag 'header'
				...
				for (var x1:int = 0; x1 < xMax; x1++) 
				{
					...
					for (var y2:int = 0; y2 < yLimit; y2++) 
					{
						...
						for (var x2:int = 0; x2 < xLimit; x2++) 
						{
							...
							block.writeByte( p & 0xfc); 		// blue	
							block.writeByte( p >> 8 & 0xfc ); 	// green
							block.writeByte( p >> 16 & 0xfc ); 		// red
						}
					}
					block.compress();
